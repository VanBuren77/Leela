...
def _load_via_sql(config: FannieLoadConfig):
    unzip_command = f"""7z e "{config.zip_file_path}" "-o{config.TEMP_ZIP_OUT_DIR}" """
    ...
    delete_command = f"""del {config.csv_file_path}"""
    ...
    if config.MAX_FILE_LOAD < FILE_LOAD_COUNTER:
        return
    ...
    for file_name in file_list:
        ...
        if ".zip" in zip_file_path and "sample" not in zip_file_path:
            config = FannieLoadConfig(
                ...
                FILE_LOAD_COUNTER=FILE_LOAD_COUNTER,
                DEBUG=DEBUG
            )
            _method(config)
